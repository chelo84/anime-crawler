...
class MALItemParser:
    ...
    def parse_anime(response):
        ...
        try:
            item['episodes'] = int(MALItemParser.__reduce_str(response.xpath('//div/span[@class="dark_text" and text()="Episodes:"]/../text()').getall()))
        except ValueError:
            item['episodes'] = 0
        ...
        try:
            item['score'] = float(ExtractUtils.extract_x_path_default_blank(response, '//div[contains(@class, "anime-detail-header-stats")]/div[contains(@class, "stats-block")]/div[@data-title="score"]/text()'))
        except ValueError:
            item['score'] = 0.00
        try:
            scored_by = ExtractUtils.extract_x_path_default_blank(response, '//div[contains(@class, "anime-detail-header-stats")]/div[contains(@class, "stats-block")]/div[@data-title="score"]/@data-user')
            item['scored_by'] = int(scored_by.replace('users', '').replace(',', '').strip())
        except ValueError:
            item['scored_by'] = 0
        try:
            item['ranked'] = int(response.xpath('//div[contains(@class, "anime-detail-header-stats")]/div[contains(@class, "stats-block")]//span[contains(@class, "ranked")]/strong/text()').get(default='N/A').replace('#', ''))
        except ValueError:
            item['ranked'] = 0
